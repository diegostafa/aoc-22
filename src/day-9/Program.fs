...
module Part1 =
    ...
    let makeMove (knots, grid) ((dirx, diry), steps) =
        List.fold
            (fun (ks, grid) _ ->
                let newKnots, _ =
                    List.fold
    ...
    let solve =
        match
            ...
            |> Seq.toList
            |> List.map toPair
            |> List.fold makeMove ((List.replicate 2 (0, 0)), (updateStepCount Map.empty (0, 0)))
        with
        | (_, g) -> printfn "Result: %i" (Map.count g)
...
module Part2 =
    ...
    let solve =
        match
            ...
            |> Seq.toList
            |> List.map toPair
            |> List.fold makeMove ((List.replicate 10 (0, 0)), (updateStepCount Map.empty (0, 0)))
        with
        | (_, g) -> printfn "Result: %i" (Map.count g)
